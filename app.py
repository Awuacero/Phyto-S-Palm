...
import tempfile
import sys
import requests  # Añadido para hacer la petición a GitHub
...
if use_sample:
    try:
        # Petición a GitHub para obtener el archivo FASTA dinámicamente
        url = "https://raw.githubusercontent.com/Awuacero/Phyto-S-Palm/main/sample.fasta"
        response = requests.get(url)
        response.raise_for_status()
        
        stringio = io.StringIO(response.text)
        sequences = list(SeqIO.parse(stringio, "fasta"))
    except Exception as e:
        st.error(f"Error downloading the sample FASTA from GitHub: {e}")
...
# Logo ajustado para usar la máxima resolución y tamaño completo (max-width: 100%)
st.markdown("<p align='center'><img src='https://raw.githubusercontent.com/Awuacero/Phyto-S-Palm/main/logos.jpg' alt='Institution Logos' style='width: 100%; max-width: 100%;'/></p>", unsafe_allow_html=True)
st.markdown("---")
